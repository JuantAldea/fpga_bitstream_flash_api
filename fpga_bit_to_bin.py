...
"""
This file has been taken from The Internet
"""
...
def convert_bit_to_bin(bitfile_path, binfile_path, flip):
    short = struct.Struct(">H")
    ulong = struct.Struct(">I")

    bitfile = open(bitfile_path, "rb")
    ...
    if b"PARTIAL=TRUE" in d:
        ...
        for i in range(len(bitfile_path) - 1, 0, -1):
            if bitfile_path[i].isdigit():
                pos_end = i + 1
                for j in range(i - 1, 0, -1):
                    if not bitfile_path[j].isdigit():
                        pos_start = j + 1
                        break
                break
        if pos_end != 0 and pos_end != 0:
            node_nr = int(bitfile_path[pos_start:pos_end])
        else:
            node_nr = 0
...
if __name__ == "__main__":
    ...
    convert_bit_to_bin(args.bitfile, args.binfile, args.flip)
